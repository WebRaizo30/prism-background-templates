import type { Template3Config } from './template3Types';

function assetUrl(file: 'style.css' | 'script.js'): string {
  const base = import.meta.env.BASE_URL || '/';
  const normalized = base.endsWith('/') ? base : `${base}/`;
  return `${normalized}templates/3/dist/${file}`;
}
...
export async function buildTemplate3StandaloneHtml(state: Template3Config): Promise<string> {
...
<head>
  ...
  <title>Sparkling Boxes — Prism export</title>
  <style>
${css}
  </style>
</head>
