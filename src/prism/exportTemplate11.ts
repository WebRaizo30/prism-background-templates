import type { Template11Config } from './template11Types';

function assetUrl(file: 'style.css' | 'script.js'): string {
  const base = import.meta.env.BASE_URL || '/';
  const normalized = base.endsWith('/') ? base : `${base}/`;
  return `${normalized}templates/11/dist/${file}`;
}
...
export async function buildTemplate11StandaloneHtml(state: Template11Config): Promise<string> {
...
<head>
  ...
  <title>Cloudy Spiral — Prism export</title>
  <style>
${css}
  </style>
</head>
